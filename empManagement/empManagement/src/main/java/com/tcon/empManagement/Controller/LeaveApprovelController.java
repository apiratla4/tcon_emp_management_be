...
public class LeaveApprovelController {
    ...
    // This endpoint already handles rejectReason perfectly
    ...
    public LeaveApprovelResponse updateStatus(@PathVariable String id, @Valid @RequestBody LeaveApprovelUpdateStatusRequest req) {
        ...
        return service.updateStatus(id, req);  // ✅ Passes rejectReason to service
    }
    ...
    @GetMapping("/view")
    public List<LeaveApprovelResponse> getLeavesForRole(
            @RequestParam String role,
            @RequestParam String empId   // logged-in user's empId
    ) {
        log.info("GET /api/leave-approvel/view role={}, empId={}", role, empId);
        return service.getLeavesForRole(role, empId);
    }

    // Update leave details (employee can edit PENDING requests)
    @PutMapping("/{id}")
    public LeaveApprovelResponse updateLeave(@PathVariable String id, @Valid @RequestBody LeaveApprovelCreateRequest req) {
        log.info("PUT /api/leave-approvel/{} empId={}", id, req.getEmpId());
        return service.updateLeave(id, req);
    }
